class PacerScene extends Phaser.Scene {
  ...
  create() {
    ...
    const selectedIndex=this.registry.get('selectedRiderIndex');
    if(Number.isInteger(selectedIndex)){
      this.playerIndex=selectedIndex;
    }

    this.fateOrder=this.registry.get('fateOrder');
    if(!Array.isArray(this.fateOrder)||this.fateOrder.length!==7){
      this.fateOrder=[0,1,2,3,4,5,6];
    }
    this.makeBackground();
    this.drawTrack();
    this.makeHUD();
    ...
    this.showCenterMessage(
      'FOLLOW DEATH',
      '1 lap with Death Pacer.\n← → = cadence\nStay in Flow and save stamina.\n\nPRESS SPACE TO START'
    );
  }
  ...
  resetGame(){
    ...
    this.riders.forEach((r,i)=>{
      ...
      r.progress=this.pacerProgress-spacing*(slot+1);
      r.speedKmh=35;
      r.cadence=82;
      r.lane=1;
      r.laneVisual=1;
      r.laneTarget=1;
      r.decisionTimer=.8+Math.random()*1.4;
    });
  ...
  beginOpenRace(){
    this.running=false;
    this.showCenterMessage('DEATH IS LEAVING','2 open race laps.\n↑ outer line   ↓ inner line\nOvertake — do not ride through rivals.');
  ...
  finishRace(winner){
    ...
    this.showCenterMessage(winner===this.riders[this.playerIndex]?'YOU WIN':'FINISH',`POSITION ${pos}/7\nSTAMINA: ${Math.round(this.stamina)}%\n\nR TO RESTART`);
  }
  ...
  bindInput(){
    ...
    this.input.keyboard.on('keydown-R',()=>{
      ...
      this.showCenterMessage('FOLLOW DEATH','1 lap with Death Pacer.\n← → = cadence\nSave stamina for the two open laps.\n\nPRESS SPACE TO START');
    });
  }
