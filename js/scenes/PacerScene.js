class PacerScene extends Phaser.Scene {
  ...
  updateOpenRace(dt){
    ...
    if(inDraft&&this.flow>=70){
      cost=this.cadence<=104?.12:.55;

    }else if(inDraft){
      cost=this.cadence<=104?.45:1.1;

    }else if(this.flow>=55){
      cost=this.cadence<=104?1.15:2.1;

    }else{
      cost=this.cadence<=104?2.1:3.6;
    }

    if(this.cadence>116){
      cost+=2.2+(this.cadence-116)*.10;
    }
    ...
    if(this.stamina>1){
      if(inDraft&&this.flow>=70){
        this.tipText.setText('DRAFT — FLOW + STAMINA');
        this.tipText.setColor('#22d9ff');

      }else if(inDraft){
        this.tipText.setText('DRAFT — FIND THE RHYTHM');
