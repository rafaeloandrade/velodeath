class PacerScene extends Phaser.Scene {
  ...
  beginOpenRace(){
    ...
    this.time.delayedCall(2200,()=>{
      ...
      // Each rider must cross the WHITE line twice after the race opens.
      this.riders.forEach((r,i)=>{
        r.openCrossings=0;
        r.nextWhiteLine=this.nextFinishCrossing(r.progress);
        r.lane=1;
        r.laneVisual=1;
        r.laneTarget=1;

        if(i!==this.playerIndex){
          r.speedKmh=40+Math.random()*3;
          r.cadence=92+Math.random()*10;
        }
      });
  ...
  updateOpenRace(dt){
    ...
    const draftTarget=this.findDraftTarget(this.playerIndex);
    const inDraft=!!draftTarget;

    if(inDraft){
      this.flow=Math.min(100,this.flow+2.0*dt);
    }else{
      this.flow=Math.max(0,this.flow-2.2*dt);
    }

    let cost;

    if(inDraft&&this.flow>=65){
      cost=this.cadence<=105?.15:.8;
    }else if(this.flow>=50){
      cost=this.cadence<=105?.9:1.8;
    }else{
      cost=this.cadence<=105?1.8:3.0;
    }

    if(this.cadence>118){
      cost+=3.2+(this.cadence-118)*.10;
    }
    ...
    this.riders.forEach(r=>{
      while(
        r.openCrossings<this.totalOpenLaps &&
        r.progress+1e-9>=r.nextWhiteLine
      ){
        r.openCrossings++;
        r.nextWhiteLine+=1;

        if(r.openCrossings>=this.totalOpenLaps&&winner===null){
          winner=r;
        }
      }
    });
    ...
    const lap=Math.min(
      2,
      Math.max(1,player.openCrossings+1)
    );
    ...
    if(this.stamina<=1){
      this.tipText.setText('EXHAUSTED — MAX ~34 km/h');
  ...
  findDraftTarget(index){
    ...
    const threshold=42/this.perimeter(this.track.referenceR);
    let nearest=null,deltaMin=Infinity;

    this.riders.forEach((other,j)=>{
      if(j===index||other.lane!==rider.lane)return;
      const d=other.progress-rider.progress;

      if(d>0&&d<threshold&&d<deltaMin){
        nearest=other;
        deltaMin=d;
      }
    });
