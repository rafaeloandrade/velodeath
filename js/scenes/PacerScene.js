class PacerScene extends Phaser.Scene {
  ...
  beginOpenRace(){
    ...
    this.time.delayedCall(2200,()=>{
      ...
      // Two FULL open-race laps; the WHITE line is the official finish.
      this.openRaceFirstLine=this.nextFinishCrossing(player.progress);
      this.openRaceFinishProgress=this.openRaceFirstLine+1;
  ...
  updateOpenRace(dt){
    ...
    const finalLapStart=this.openRaceFinishProgress-1;
    const lap=player.progress<finalLapStart?1:2;
    ...
    const winner=this.riders.find(r=>r.progress+1e-9>=this.openRaceFinishProgress);
