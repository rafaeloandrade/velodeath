class PacerScene extends Phaser.Scene {
  ...
  resetRun(){
    this.running=false;this.failed=false;this.pacerDistance=120;this.pacerTravel=0;this.pacerKmh=33;this.playerDistance=this.pacerDistance-(this.playerSlot+1)*34;this.playerSpeed=this.kmhToPixels(33); this.stamina=100;this.flow=45;this.cadence=72;this.lastPedalKey=null;this.lastPedalTime=0;this.intervals=[];this.currentLap=1;this.updateFormationPositions();this.updateHUD();
  }
  kmhToPixels(k){return k*3.25} pixelsToKmh(px){return px/3.25}
  targetPacerKmh(){const p=Phaser.Math.Clamp(this.pacerTravel/(this.perimeter(this.track.rideR)*4),0,1);return Phaser.Math.Linear(33,44,p)}
  handlePedal(code){
  ...
  cadenceTargetKmh(){
    const rpm=this.cadence;
    if(rpm<35)return 30;
    if(rpm<55)return Phaser.Math.Linear(30,34,(rpm-35)/20);
    if(rpm<75)return Phaser.Math.Linear(34,39,(rpm-55)/20);
    if(rpm<95)return Phaser.Math.Linear(39,45,(rpm-75)/20);
    if(rpm<115)return Phaser.Math.Linear(45,50,(rpm-95)/20);
    return Phaser.Math.Linear(50,55,Phaser.Math.Clamp((rpm-115)/25,0,1));
  }
...
R para reiniciar'
  ...
  updateHUD(gap=0){
    ...
    if(gap>235){this.tipText.setText('WARNING — GAP CRÍTICO');this.tipText.setColor('#ff5a5a')}
    else if(gap>145){this.tipText.setText('GAP — aumente o ritmo');this.tipText.setColor('#ffcc33')}
    ...
    else{this.tipText.setText('← → 75–95 RPM = ZONA EFICIENTE');this.tipText.setColor('#9ba4b7')}
  }
