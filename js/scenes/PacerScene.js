class PacerScene extends Phaser.Scene {
  ...
  create(){
    ...
    this.showCenterMessage('FOLLOW DEATH','Alterne ← → em ritmo constante\n49x14: ~80–100 rpm acompanha o pacer com economia\n\nESPAÇO para começar');
  }
  ...
  makeHUD(){
    ...
    this.tipText=this.add.text(480,620,'← → PEDAL',{fontFamily:'monospace',fontSize:'12px',color:'#9ba4b7'}).setOrigin(.5);this.add.text(925,620,'GEAR 49x14',{fontFamily:'monospace',fontSize:'11px',color:'#22d9ff'}).setOrigin(1,.5);
  }
  ...
  resetRun(){
    this.running=false;this.failed=false;this.pacerDistance=120;this.pacerTravel=0;this.pacerKmh=35;this.playerDistance=this.pacerDistance-(this.playerSlot+1)*34;this.playerSpeed=this.kmhToPixels(35); this.stamina=100;this.flow=72;this.cadence=80;this.lastPedalKey=null;this.lastPedalTime=0;this.intervals=[];this.currentLap=1;this.updateFormationPositions();this.updateHUD();
  }
  ...
  targetPacerKmh(){
    ...
    return Phaser.Math.Linear(41.5,43,Phaser.Math.Clamp(lap-3,0,1));
  }
  handlePedal(code){
    if(!this.running||this.failed)return;
    const now=this.time.now;
    if(this.lastPedalKey===code){this.flow=Math.max(0,this.flow-3);return}

    if(this.lastPedalTime>0){
      const int=now-this.lastPedalTime;
      if(int>=120&&int<1600){
        this.intervals.push(int);
        if(this.intervals.length>6)this.intervals.shift();

        const avg=this.intervals.reduce((a,b)=>a+b,0)/this.intervals.length;
        // 500ms ≈ 60rpm, 375ms ≈ 80rpm, 300ms ≈ 100rpm
        this.cadence=Phaser.Math.Clamp(30000/avg,0,145);

        const variance=this.intervals.reduce((s,x)=>s+Math.abs(x-avg),0)/this.intervals.length;
        const consistency=Phaser.Math.Clamp(1-variance/180,0,1);

        this.flow=Phaser.Math.Clamp(
          this.flow+3.2*consistency-.6*(1-consistency),
          0,100
        );
      }
    }
  ...
  cadenceTargetKmh(){
    /*
      Realistic fixed-gear speed model.
      Current prototype uses 49x14 as the default bike.
      Wheel circumference approximation: 2.096 m (700C).
    */
    const front=49;
    const rear=14;
    ...
    // km/h = cadence * gear ratio * wheel circumference * 60 / 1000
    return this.cadence * gearRatio * wheelCircumference * 60 / 1000;
  }

  updateGameplay(dt){
    if(!this.running||this.failed)return;

    // Death Pacer: 4 laps, progressive but approachable.
    const target=this.targetPacerKmh();
    this.pacerKmh=Phaser.Math.Linear(this.pacerKmh,target,.55*dt);
    const pacerSpeed=this.kmhToPixels(this.pacerKmh);
    this.pacerDistance+=pacerSpeed*dt;
    this.pacerTravel+=pacerSpeed*dt;

    // NEW MODEL: cadence sets a target speed.
    // No more tiny impulse + heavy decay every frame.
    const targetKmhBase=this.cadenceTargetKmh();
    const flowBonus=Phaser.Math.Linear(0,1.8,this.flow/100);
    const staminaPenalty=this.stamina<30
      ? Phaser.Math.Linear(3.5,0,this.stamina/30)
      : 0;

    const targetKmh=Math.max(28,targetKmhBase+flowBonus-staminaPenalty);
    const targetPx=this.kmhToPixels(targetKmh);

    // Bicycle inertia: quicker to accelerate, slower to lose speed.
    const accel=this.kmhToPixels(10);
    const decel=this.kmhToPixels(4.5);

    if(this.playerSpeed<targetPx){
      this.playerSpeed=Math.min(targetPx,this.playerSpeed+accel*dt);
    }else{
      this.playerSpeed=Math.max(targetPx,this.playerSpeed-decel*dt);
    }

    const playerKmh=this.pixelsToKmh(this.playerSpeed);

    // Stamina reacts to sustained effort, not frantic clicking.
    if(this.cadence>108||playerKmh>48){
      const effort=Math.max(
        (this.cadence-105)/45,
        (playerKmh-47)/10
      );
      this.stamina-=(1.7+Math.max(0,effort)*3.2)*dt;
    }else if(this.cadence<82&&playerKmh<42){
      this.stamina+=4.8*dt;
    }else{
      this.stamina+=.8*dt;
    }
    ...
    // Cadence remains stable briefly after the last press.
    const idle=this.time.now-this.lastPedalTime;
    if(idle>900){
      this.cadence=Math.max(0,this.cadence-22*dt);
      this.flow=Math.max(0,this.flow-.8*dt);
    }

    this.playerDistance+=this.playerSpeed*dt;
    this.updateFormationPositions();

    const desired=this.pacerDistance-(this.playerSlot+1)*34;
    const gap=desired-this.playerDistance;

    // Prevent riding through the wheel ahead.
    if(gap<-10){
      this.playerDistance=desired+10;
      this.playerSpeed=Math.min(this.playerSpeed,pacerSpeed*1.02);
    }

    // More forgiving recovery window.
    if(gap>360)this.failRun();

    this.currentLap=Math.min(
      4,
      1+Math.floor(this.pacerTravel/this.perimeter(this.track.rideR))
    );

    if(this.pacerTravel>=this.perimeter(this.track.rideR)*4){
      this.running=false;
...
R para reiniciar'
  ...
  updateHUD(gap=0){
    ...
    this.staminaBar.width=124*(this.stamina/100);this.flowBar.width=124*(this.flow/100);this.phaseText.setText(`DEATH PACER // LAP ${this.currentLap}/4`);
    if(gap>280){this.tipText.setText('WARNING — GAP CRÍTICO');this.tipText.setColor('#ff5a5a')}
    else if(gap>180){this.tipText.setText('GAP — aumente o ritmo');this.tipText.setColor('#ffcc33')}
    else if(this.stamina<25){this.tipText.setText('LOW STAMINA — reduza a cadência');this.tipText.setColor('#ffcc33')}
    else if(this.flow>75){this.tipText.setText('FLOW — ritmo sincronizado');this.tipText.setColor('#ff2b6d')}
    else{this.tipText.setText('49x14  //  80–100 RPM = PACER ZONE');this.tipText.setColor('#9ba4b7')}
  }
  ...
  bindInput(){
    ...
    this.input.keyboard.on('keydown-R',()=>{this.resetRun();this.showCenterMessage('FOLLOW DEATH','Alterne ← → em ritmo constante\n49x14: ~80–100 rpm acompanha o pacer com economia\n\nESPAÇO para começar')});
